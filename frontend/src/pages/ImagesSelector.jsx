...
const ImageSelector = ({ image, setImage, handleDeleteImg }) => {
    ...
    useEffect(() => {
        //if the image is string URL set as preview URL
        if (typeof image === 'string') {
            setPreviewUrl(image);
        } else if (image) {
            //if the image is file object create new url
            setPreviewUrl(URL.createObjectURL(image));
        } else {
            //if no image clear preview url
            setPreviewUrl(null);
        }
        return () => {
        ...
        <div>
            ...
            >
                <div className="w-14 h-14 flex items-center justify-center bg-gray-50 rounded-full border border-gray-100">
                    <FaRegFileImage className="text-xl text-gray-500" />
                </div>
